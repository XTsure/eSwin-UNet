...
class MulOutput(nn.Module):
    def __init__(self, in_channels, tran_out_channels, out_channels, scale=2):
        super(MulOutput, self).__init__()

        self.tran_conv = nn.ConvTranspose2d(in_channels, tran_out_channels, kernel_size=scale, stride=scale)
        self.mul_out = self.out = nn.Conv2d(tran_out_channels, out_channels, 1)
...
class UNetMul(nn.Module):
    '''UNet 网络架构
    lr 0.001
    '''

    def __init__(self, img_channel, class_num):
        super().__init__()

        self.conv_block_1 = DoubleConvBlock(img_channel, 32)
        ...
        self.mul_out_layer = nn.ModuleList()
        for out_layer in range(3):
            mul_output = MulOutput(32 * 2 ** (3 - out_layer), 32 * 2 ** (2 - out_layer), class_num,
                                   2 ** (3 - out_layer))
            self.mul_out_layer.append(mul_output)
    ...
    def forward(self, x):
        ...
        up_list = [conv_block_out_6, conv_block_out_7, conv_block_out_8]
        out = []
        for out_index, out_layer in enumerate(self.mul_out_layer):
            mul_out = out_layer(up_list[out_index])
            out.append(mul_out)

        final_out = self.out(conv_block_out_9)
