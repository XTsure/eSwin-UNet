...
class DoubleConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True, feature_size=224):
        super(DoubleConvBlock, self).__init__()

        self.patch_num = 7
        self.dim = (feature_size // self.patch_num) ** 2 * in_channels
        self.mlp_dim = 2 * self.dim

        self.vit = ViT(image_size=feature_size, patch_num=self.patch_num, dim=self.dim, depth=3, heads=12,
                       mlp_dim=self.mlp_dim, dropout=0.1, emb_dropout=0.1, channels=in_channels)
    ...
    def forward(self, x):
        vit_out = self.vit(x)
        conv_out_1 = self.conv_1(vit_out)
...
class VitInBaseUNet(nn.Module):
    '''UNet 网络架构
    lr 0.001
    '''

    def __init__(self, img_size, img_channel, class_num):
        super().__init__()

        self.conv_block_1 = DoubleConvBlock(img_channel, 32, feature_size=224)
        self.down_sampling_1 = nn.MaxPool2d(2, 2)
        self.conv_block_2 = DoubleConvBlock(32, 64, feature_size=112)
        self.down_sampling_2 = nn.MaxPool2d(2, 2)
        self.conv_block_3 = DoubleConvBlock(64, 128, feature_size=56)
        self.down_sampling_3 = nn.MaxPool2d(2, 2)
        self.conv_block_4 = DoubleConvBlock(128, 256, feature_size=28)
        self.down_sampling_4 = nn.MaxPool2d(2, 2)
        self.conv_block_5 = DoubleConvBlock(256, 512, feature_size=14)
        self.up_sampling_1 = UpSamplingBlock(512, 256)
        self.conv_block_6 = DoubleConvBlock(512, 256, feature_size=28)
        self.up_sampling_2 = UpSamplingBlock(256, 128)
        self.conv_block_7 = DoubleConvBlock(256, 128, feature_size=56)
        self.up_sampling_3 = UpSamplingBlock(128, 64)
        self.conv_block_8 = DoubleConvBlock(128, 64, feature_size=112)
        self.up_sampling_4 = UpSamplingBlock(64, 32)
        self.conv_block_9 = DoubleConvBlock(64, 32, feature_size=224)
        self.out = nn.Conv2d(32, class_num, 1)
